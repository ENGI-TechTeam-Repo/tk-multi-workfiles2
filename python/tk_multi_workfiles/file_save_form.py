...
class FileSaveForm(FileFormBase):
    ...
    def _do_init(self):
        ...
        self._ui.file_name_preview.setText(
            "<p style='color:rgb%s'></p>" % (self._preview_colour,)
        )
        self._ui.work_area_label.setText(
            "<p style='color:rgb%s'><b>Work Area:</b></p>" % (self._preview_colour,)
        )
        self._ui.work_area_preview.setText(
            "<p style='color:rgb%s'></p>" % (self._preview_colour,)
        )
        self._ui.warning_label.setText(
            "<p style='color:rgb%s'><b>Warning:</b></p>" % (app.warning_color,)
        )
        self._set_warning("")
    ...
    def _set_warning(self, reason):
        ...
        self._ui.warning.setText(
            "<p style='color:rgb%s'>%s</p>" % (app.warning_color, reason)
        )
    ...
    def _start_preview_update(self):
        ...
        self._disable_save(
            "Please wait while Toolkit calculates the next available file name."
        )
        ...
        self._preview_task = self._bg_task_manager.add_task(
            self._generate_path,
            priority=35,
            task_kwargs={
    ...
    def _on_preview_generation_complete(self, task_id, group, result):
        ...
        if path is None:
            ...
            name_preview = (
                "Unable to generate preview - this may be because nothing has "
        ...
        self._ui.feedback_stacked_widget.setCurrentWidget(self._ui.preview_page)
        self._ui.file_name_preview.setText(
            "<p style='color:rgb%s'>%s</p>" % (self._preview_colour, name_preview)
        )
        self._ui.work_area_preview.setText(
    ...
    def _update_extension_menu(self, ext, block_signals=True):
        ...
        try:
            if ext in self._extension_choices:
                self._ui.file_type_menu.setCurrentIndex(
                    self._extension_choices.index(ext)
                )
        finally:
        ...
        try:
            if not file.is_local and file.is_published:
                if self._current_env.publish_template:
                    fields = self._current_env.publish_template.get_fields(
                        file.publish_path
                    )
            else:
    ...
    def _on_save(self):
        ...
        try:
            ...
            try:
                SaveAsFileAction.create_folders_if_needed(
                    self._current_env.context, self._current_env.work_template
                )
            except TankError as e:
                app.log_exception(
                    "File Save - failed to create folders for context '%s'!"
            ...
            if (
                ...
                answer = QtGui.QMessageBox.question(
                    self,
                    "Save File as Version v%03d?" % version_to_save,
                    msg,
